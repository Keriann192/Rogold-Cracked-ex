...
pages.friends = async () => {
    ...
    let maxBF = await get(`https://inventory.roblox.com/v1/users/${await getUserId()}/items/GamePass/26817185`) // Very mean if you change this ;( its cheap too
    ...
    const friendLoad = async (friend) => {
        ...
        if (status) {
            ...
            let activity = await getPresence(id)
            if (activity.LastLocation.search('Creating') !== -1 && activity.PlaceId && activity.LocationType == 6) {
                ...
                if (disabled && avatarName && avatarName.tagName == "DIV") { // actually disabled or banned
                    ...
                    replacement.setAttribute('href', `/banned-users/` + id)
