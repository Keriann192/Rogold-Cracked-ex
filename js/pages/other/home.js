...
async function addGame(name, id, url, thumbnail, num) {
    ...
    if (!canEdit || canEdit == undefined || canEdit.errors || canEdit == "ERROR" || !canEdit.id || isMobile()) {
...
pages.home = (async () => {
        ...
        getSetting('Pinned Games').then(async (result) => {
            if (result) {
                ...
                const addPinned = async (added_node) => {
                    ...
                    didPinned = true
                    if (!added_node) {
                        added_node = await first(".game-home-page-container div:first-child")
                    }
                    let pinTitle = document.createElement('div')
                    pinTitle.setAttribute('class', 'col-xs-12 container-list places-list')
                    pinTitle.setAttribute('style', 'margin-top: 6px; height: auto')
                    ...
                    if (maxPinned?.data?.[0]) {
                        ...
                        <a>
                            <div class="container-header games-filter-changer">
                                <h3>${chrome.i18n.getMessage("pinnedGames") || "Pinned"}📌</h3>
                            </div>
                        </a>
                    ...
                    syncGet('pinned', async function (result) {
                        if (result && result.pinned && result.pinned.length > 0) {
                            const continueAddNode = async () => {
                                if (!added_node.parentNode) added_node = qs(".game-home-page-container div:first-child");
                                ...
                                for (const gameInfo of gameInfos?.data) {
                                    ...
                                    num++
                                    let thumb = thumbnails.data.find(x => x.targetId == gameInfo.id)?.imageUrl
                                    addGame(gameInfo.name, gameInfo.rootPlaceId, "https://roblox.com/games/" + gameInfo.rootPlaceId, thumb, num)
                                }
                            }
                            if (!added_node) {
                                added_node = document.querySelector(".game-home-page-container div:first-child")
                            }
                            if (!added_node) {
                                setTimeout(() => {
                                    added_node = document.querySelector(".game-home-page-container div:first-child")
                ...
                first(".game-home-page-container div:first-child", addPinned)
    ...
    angularScript.src = chrome.runtime.getURL("/js/useAngular.js");
    ...
    const createHoverPopup = async (activator, userInfo, userStatus) => {
        ...
        element.className = `popover bottom people-info-card-container ${userStatus.PlaceId && "card-with-game" || ""} fade`
        ...
        const gameInfo = userStatus.PlaceId && (await get('https://games.roblox.com/v1/games/multiget-place-details?placeIds=' + userStatus.PlaceId))?.[0] || {}
        const gameThumbnail = userStatus.PlaceId && (await get('https://thumbnails.roblox.com/v1/games/icons?universeIds=' + gameInfo?.universeId + "&size=150x150&format=Png&isCircular=false"))?.data || []
        element.innerHTML = `
        <div class="arrow"></div>
        <div class="popover-inner">
            <div class="popover-content" uib-tooltip-template-transclude="contentExp()" tooltip-template-transclude-scope="originScope()">
                <div ng-controller="peopleInfoCardController" ng-class="{'card-with-game': friend.presence.placeUrl}" class="ng-scope ${userStatus.PlaceId == null && "card-with-game" || ""}"> 
                    <div class="border-bottom place-container ${userStatus.PlaceId == null && "ng-hide" || ""}"> 
                        <span ng-click="goToGameDetails('icon')" onclick="window.location.href='/games/${userStatus.PlaceId}'"> 
                            <thumbnail-2d class="cursor-pointer place-icon ng-isolate-scope" thumbnail-type="layout.thumbnailTypes.gameIcon" thumbnail-target-id="library.placesDict[friend.presence.rootPlaceId].universeId">
                                <span ng-class="$ctrl.getCssClasses()" class="thumbnail-2d-container" thumbnail-type="GameIcon" thumbnail-target-id="">
                                    <img image-load="" alt="" title="" class="ng-scope ng-isolate-scope" src="${gameThumbnail && gameThumbnail?.find(e => e.targetId == gameInfo.universeId)?.imageUrl || "https://t4.rbxcdn.com/3f73af996b69eafb3df5770a07050f43"}">
                                </span> 
                            </thumbnail-2d> 
                        </span> 
                        <div class="place-info-container"> 
                            <div class="place-info"> 
                                <span class="text-subject cursor-pointer place-title" onclick="window.location.href='/games/${userStatus.PlaceId}'">${gameInfo?.name}</span> 
                                <div class="icon-text-wrapper ${gameInfo?.price == 0 && "ng-hide" || ""}"> 
                                    <span class="icon-robux"></span> 
                                    <span class="text-robux ng-binding">${gameInfo?.price}</span> 
                                </div> 
                            </div> 
                            <div class="place-btn-container"> 
                                <button class="btn-full-width place-btn btn-control-sm" style="display: ${gameInfo?.isPlayable ? "none" : "initial"}" onclick="window.location.href='/games/${userStatus.PlaceId}'"> View Details </button>
                                <button class="btn-full-width place-btn btn-growth-sm" style="display: ${gameInfo?.isPlayable ? "initial" : "none"}" onclick="Roblox.GameLauncher.joinMultiplayerGame(${gameInfo.universeRootPlaceId})"> Join </button>
                            </div> 
                        </div>
                    </div>
                    <ul class="dropdown-menu interaction-container"> 
                        <li class="interaction-item" ng-click="goToChat()"> 
                            ...
                            <span class="text-overflow border-bottom label ng-binding" ng-bind="layout.interactionLabels.chat(friend.nameToDisplay)" title="Chat with ${userInfo.name}">Chat with ${userInfo.name}</span> 
                        </li> 
                        <li class="interaction-item" ng-click="goToProfilePage()"> 
                            <span class="icon icon-viewdetails"></span> 
                            <span class="label ng-binding" onclick="window.location.href='/users/${userInfo.id}/profile'">View Profile</span> 
                        </li> 
                    </ul> 
                </div>
            </div>
        ...
        qs(".dropdown-menu .text-overflow", element).addEventListener("click", async () => {
            let foundChat = qs(`.chat-avatar-headshot[user-id="${userInfo.id}"]`)?.parentNode
            if (foundChat) {
                window.postMessage({
                    type: "angular",
                    data: {
                ...
                qs(".chat-search-input").value = userInfo.name
                qs(".chat-search-input").dispatchEvent(new Event("input"))
                qs(".chat-search-input").dispatchEvent(new Event("keyup"))
                setTimeout(async () => {
                    foundChat = qs(`.chat-avatar-headshot[user-id="${userInfo.id}"]`)?.parentNode
                    if (foundChat) {
                        window.postMessage({
                            type: "angular",
                            data: {
        ...
        activator.addEventListener('mouseenter', (e) => {
            ...
            element.style.left = activator.getBoundingClientRect().left + (activator.offsetWidth / 2) - (element.offsetWidth / 2) + "px"
            element.style.top = activator.getBoundingClientRect().top + scrollY + (activator.offsetHeight * 1.1) + "px"
        })
    ...
    setTimeout(async () => {
        ...
        let maxBF = await get(`https://inventory.roblox.com/v1/users/${userId}/items/GamePass/26817185`) // Very mean if you change this ;( its cheap too
        ...
        if (favorites && favorites.length > 0) {
            ...
            <div class="col-xs-12 people-list-container" style="height:auto">
                <div class="section home-friends" style="height:auto">
                    <a class="container-header people-list-header" ${maxNum == 9 ? 'href="/game-pass/26817185/9-Best-Friends"' : ''}>
                        <h3>${chrome.i18n.getMessage("bestFriends") || "Best Friends"}<span class="friends-count ng-binding">(${favorites.length}/${maxNum})</span> </h3>
            ...
            `;
            (await first("#HomeContainer")).insertBefore(container, (await first("#HomeContainer")).children[1])
            ...
            for (const friend of favFriends) {
                setTimeout(async () => {
                    ...
                    friendElement.querySelector("a").href = `/users/${friend.id}/profile`
                    friendElement.querySelector("img").src = thumbnail
                    friendElement.querySelector(".friend-name").innerText = stripTags(friend.displayName)
                    friendElement.querySelector(".friend-name").title = stripTags(friend.displayName)
                    container.querySelector('.hlist').appendChild(friendElement)
                    let activity = await getPresence(friend.id)
                    createHoverPopup(qs(".friend-link", friendElement), friend, activity)
                    ...
                    if (activity?.IsOnline) {
                        ...
                        placeName.innerHTML = `<div class="text-overflow xsmall text-label place-name">${dateSince(new Date(activity.LastOnline))}</div>`
                        friendStatus.remove()
                    }
                }, 0)
            }
    ...
    setTimeout(async () => {
        ...
        `;
        (await first("#place-list .game-home-page-container > div:nth-child(5)")).before(favorites)
            ...
            pager.onset = (newPage) => {
                ...
                for (const Item of page) {
                    ...
                    itemHtml.innerHTML = `
                        ...
                        <a class="item-card-link" href="${stripTags(Item.Item.AbsoluteUrl)}">
                        ...
                        <div ng-if="$ctrl.showCreatorName" class="text-overflow item-card-label ng-scope"> 
                            ...
                            <a class="creator-name text-overflow text-link" href="${stripTags(Item.Creator.CreatorProfileLink)}" style="display: inline;">${stripTags(Item.Creator.Name)}</a> 
    ...
    first(".list-item.friend .text-overflow.xsmall.text-label.place-name", async () => {
        ...
        on(".list-item.friend", async() => {
            for (const friend of document.querySelectorAll('.list-item.friend') ?? []) {
                ...
                if (!playing) {
                    ...
                    let friendStatus = await getPresence(id)
                    ...
                    if (placeName && friendStatus.LastLocation && friendStatus.IsOnline) {
                        placeName.innerText = stripTags(friendStatus.LastLocation?.replace(/Playing|Creating/g, ""))
                    } else if(placeName && !friendStatus.IsOnline) {
                        placeName.innerText = dateSince(new Date(friendStatus.LastOnline))
                    }
                }
            }
        })
    ...
    getSetting("Friend Updater").then(option => {
        ...
        const popups = {}
        setInterval(async () => {
            const frequents = (await get('https://friends.roblox.com/v1/users/' + userId + '/friends?userSort=StatusFrequents'))?.data
            if (!frequents) return;
            for (let i = 0; i < 9; i++) {
                setTimeout(async () => {
                    const friend = frequents[i]
                    const list = qs("#people-list-container .people-list .hlist")
                    const element = list?.children?.[i]
                    if (!element) return;
                    if (element.getAttribute("rbx-user-id") == friend.id) return;
                    if (popups[friend.id]) popups[friend.id]();
                    const friendElement = document.createElement('li')
                    friendElement.setAttribute('class', 'list-item friend')
                    friendElement.setAttribute('rbx-user-id', friend.id)
                    friendElement.innerHTML = friendHTML
                    qs("a", friendElement).href = `/users/${friend.id}/profile`
                    qs("img", friendElement).src = `https://www.roblox.com/headshot-thumbnail/image?userId=${friend.id}&width=420&height=420&format=png`
                    qs(".friend-name", friendElement).innerText = stripTags(friend.displayName)
                    qs(".friend-name", friendElement).title = stripTags(friend.name)
                    ...
                    qs(".friend-link", friendElement).href = "/users/" + friend.id + "/profile"
                    qs(".friend-link", friendElement).setAttribute("popover-class", "people-info-card-container people-info-" + friend.id)
                    const friendStatus = await getPresence(friend.id)
                    const statusElement = qs(".friend-status", friendElement)
                    const updateElement = qs(".icon-studio, .icon-online, .icon-game", statusElement) || statusElement
                    if (statusElement && friendStatus?.IsOnline) {
                        if (friendStatus.PlaceId) {
                            statusElement.href = "https://roblox.com/games/" + friendStatus.PlaceId
                            updateElement.className = "avatar-status friend-status icon-" + (friendStatus.PresenceType == 3 && "studio" || "game")
                        } else {
                            statusElement.href = ""
                            updateElement.className = "avatar-status friend-status icon-online"
                        }
                    } else if (statusElement) {
                        statusElement.href = ""
                        updateElement.className = "avatar-status friend-status"
                    }
                    ...
                    const popup = await createHoverPopup(qs(".friend-link", friendElement), friend, friendStatus)
    ...
    const getGreeting = async () => {
        return new Promise(async resolve => {
            ...
            if (greetingType != "Off") {
                ...
                resolve((greetingType == "Dynamic" && (hours < 5 ? "Night" : hours < 12 ? "Morning" : hours < 18 ? "Afternoon" : "Evening") || greetingType) + ", ")
            }
            resolve("")
        })
    }
    if (await getSetting("Home User")) {
        on("#HomeContainer > .section > .container-header > h1", async (e) => {
            const hasPremium = (await first("#upgrade-now-button"))?.textContent == "Premium"
                        ...
                        <span class="avatar-card-link friend-avatar icon-placeholder-avatar-headshot" style="width:128px;height:128px;">
                            <thumbnail-2d class="avatar-card-image">
                                <span class="thumbnail-2d-container">
                                    <img src="${(await first("#navigation .avatar img"))?.src}" style="background-color: #d4d4d4;"></img>
                                </span>
                            </thumbnail-2d>
                        </span>
                    </a>
                    ${hasPremium && '<span class="icon-premium-medium" style="margin-right: 10px;"></span>' || ""}
                    <a href="/users/${userId}/profile" class="user-name-container">${await getGreeting()}${userData?.getAttribute("data-displayname") ?? 
                        qs("#navigation .text-nav .font-header-2")?.textContent}</a>
                `
            }
            applyStyle()
