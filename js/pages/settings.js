...
pages.settings = (async () => {
    ...
    const doSettings = async () => {
        ...
        did = true
        let hasExperiments = await get(`https://inventory.roblox.com/v1/users/12693268/items/GamePass/106133076`)
        let hasCreator = await get(`https://inventory.roblox.com/v1/users/12693268/items/GamePass/106133076`) // no change plz lol just buy it helps out a lot
        if (hasCreator) {
            hasCreator = hasCreator?.data?.[0]
        }
        let hasMoreNav = await get(`https://inventory.roblox.com/v1/users/12693268/items/GamePass/106133076`)
        if (hasMoreNav) {
            hasMoreNav = hasMoreNav?.data?.[0]
        }
        ...
        const toggle = async () => {
            qs(".menu-option.active", vertical)
                .setAttribute('class', 'menu-option ng-scope')
            qs(".user-account-header")
                .textContent = "RoGold Settings"
            ...
            const loadInfoPage = async () => {
                ...
                if (isMobile()) {
                    qs(".tab-dropdown .input-group-btn.dropdown button.dropdown-toggle span.rbx-selection-label")
                        .textContent = "Info"
                }
                page.classList.remove("hidden")
                qs(".settings-personal-container", page)
                    .innerHTML = `
                    ...
                    <div class="section-content notifications-section">
                        <div class="security-2svsetting-label btn-toggle-label"> 
                            ...
                            <a href="https://apps.apple.com/us/app/rogold/id1618599725?itsct=apps_box_badge&amp;itscg=30200" style="display: inline-block; overflow: hidden; border-radius: 13px; width: 250px; height: 83px;">
                                <img src="${chrome.runtime.getURL("images/appstore.svg")}" alt="Download on the App Store" style="border-radius: 13px; width: 250px; height: 83px;">
                            </a>
                            <br>
                            <a href="https://rogold.me/chrome" class="rogold-platform">
                                <img src="${chrome.runtime.getURL("images/chrome.png")}" alt="Install on Chrome" style="border-radius: 13px;width: 40px;height: 40px;">
                            </a>
                            <a href="https://rogold.me/edge" class="rogold-platform">
                                <img src="${chrome.runtime.getURL("images/edge.png")}" alt="Install on Edge" style="border-radius: 13px;width: 40px;height: 40px;">
                            </a>
                            <a href="https://rogold.me/firefox" class="rogold-platform">
                                <img src="${chrome.runtime.getURL("images/firefox.png")}" alt="Install on Firefox" style="border-radius: 13px;width: 40px;height: 40px;">
                            </a>
                            <a href="https://rogold.me/invite" class="rogold-platform">
                                <img src="${chrome.runtime.getURL("images/discord.svg")}" alt="Install on Firefox" style="border-radius: 13px;width: 40px;height: 40px;">
                            </a>
                        </div>
                    </div>
                    <div class="section-content notifications-section">
                ...
                first("#rogold-status", async (el) => {
                    const status = await cacheValue("StatusPage", async () => {
                        return new Promise(res => {
                            getClient("https://rogold.live/api/info/status?extension=RoGold")
                                .then(res)
                        })
                    }, 1000 * 60 * 10)
                    console.log(status)
                    ...
                    if (status) {
                        el.innerHTML = stripTags(status.message) + `<br></br><strong class='text-secondary'>${chrome.i18n.getMessage("appMessage")}</strong>`
            ...
            qs(".ng-scope", pageContent)
                .remove()
            loadInfoPage()
            const switchInfoPage = async () => {
                if (currentPage != "Info") {
                    currentPage = "Info"
                    qs(".rogold-page:not(.hidden)")
            ...
            const toggleButton = (key, bool) => {
                document.getElementById(key + "-toggle")
                    .className = `btn-toggle receiver-destination-type-toggle${bool && " on" || ""}`
            }
            ...
            Object.keys(defaultSettings)
                .forEach(async (key) => {
                    ...
                    const pageButton = createSideButton(chrome.i18n.getMessage("settings" + key) || key)
                            ...
                            Object.keys(category)
                                .forEach(async (key) => {
                                    ...
                                    `
                                        ...
                                        if (!hasExperiments && setting.experimental) {
                                            ...
                                            document.getElementById(key + "-toggle")
                                                .setAttribute('style', `opacity: 0;filter: alpha(opacity = 0);`)
                                            document.getElementById(key + "-toggle")
                                                .innerHTML = ``
                                            document.getElementById(key + "-toggle")
                                    ...
                                    `
                                        settingElement.querySelector(".tooltip-container")
                                            .addEventListener('mouseenter', (info) => {
                                                popup.setAttribute('style', `top: ${info.layerY * 1.01}px; left: ${info.layerX / 1.3}px`)
                                                settingElement.querySelector("div")
                                                    .insertBefore(popup, settingElement.querySelector(".rbx-select-group"))
                                            })
                                        settingElement.querySelector(".tooltip-container")
                                ...
                                `
                                        ...
                                        if (setting.need) {
                                            for (const need in setting.need) {
                                                if (getSetting(need) != setting.need[need]) {
                                                    document.getElementById(`${key}-name`)
                                                        .parentNode.parentNode.setAttribute('style', 'display:none;')
                                                    break
                                                }
                                            }
                                        }
                                    ...
                                    `
                                        ...
                                        function setImageBG(image, stylingInfo) {
                                            ...
                                            if (stylingInfo[2]["BG Sticky"] == "Fixed") {
                                                let bdy = document.documentElement.style
                                                bdy.setProperty("--background-image", `url(${image})`)
                                                bdy.setProperty("--background-repeat", stylingInfo[2]["BG Repeat"].toLowerCase())
                                                bdy.setProperty("--background-blend-mode", stylingInfo[2]["BG IMG Blend Mode"].toLowerCase())
                                                bdy.setProperty("--background-size", stylingInfo[2]["BG Size"].toLowerCase())
                                            } else {
                                                $("#rbx-body")
                                                    .css({
                                                        "background-image": `url(${image})`,
                                                        "background-repeat": stylingInfo[2]["BG Repeat"],
                                                        "background-blend-mode": stylingInfo[2]["BG IMG Blend Mode"],
                                                        "background-size": stylingInfo[2]["BG Size"],
                                                    })
                                            }
                                        }
                                        qs('#settings-holder', page)
                                        ...
                                        if (!currentSetting[3]?.["BG Sticky"] || !hasCreator) {
                                            currentSetting[3]["BG Repeat"] = "Repeat"
                                            currentSetting[3]["BG IMG Blend Mode"] = "Normal"
                                            currentSetting[3]["BG Size"] = "Auto"
                                            currentSetting[3]["BG Sticky"] = "Fixed"
                                        }
                                        let navigationSettings = await getSetting("Navigation Buttons")
                                        ...
                                        for (const option of setting.options) {
                                            ...
                                            const traverseList = async () => {
                                                return new Promise((resolve, reject) => {
                                                    ...
                                                    if (option.list) {
                                                        let i = 0
                                                        Object.entries(option.list)
                                                            .forEach(([name, listOption]) => {
                                                                i++
                                                                if (listOption.info) {
                                                                    addText += `
                                                                <div style="
                                                            ...
                                                            `
                                                                ...
                                                                `
                                                                    } else if (listOption.type == "toggle") {
                                                                        addText += `
                                                                ...
                                                                `
                                                                    } else if (listOption.type == "navtext") {
                                                                        addText += `
                                            ...
                                            document.getElementById(key + '-holder')
                                                .appendChild(section)
                                            section.innerHTML = `
                                        ...
                                        `
                                                    ...
                                                    syncGet(null, (gotten) => {
                                                        ...
                                                        Object.keys(gotten)
                                                            .forEach(keyGot => {
                                                                ...
                                                                if (i == Object.keys(gotten)
                                                                    .length) resolve(toRemove);
                                                                i++
                                                            })
                                                    })
                                                })
                                                    ...
                                                    .forEach(([name, item]) => {
                                                        ...
                                                        if (!item.sel) {
                                                            if (key == "Navigation Buttons") {
                                                                if (item.type == "toggle") {
                                                                    let current = navigationSettings.defaults[name]
                                                                    toggleButton(name, current)
                                                                    addToggler(name, current, () => {
                                                                        current = !current
                                                                        toggleButton(name, current)
                                                                        navigationSettings.defaults[name] = current
                                                                        setSetting("Navigation Buttons", navigationSettings)
                                                                    }, setting)
                                                                } else if (item.type == "navtext") {
                                                                    let current = navigationSettings.custom[name] ?? {
                                                                        name: "",
                                                                        url: ""
                                                                    }
                                                                    if (!hasMoreNav && Number(name.match(/\d+/)[0] || 0) > 3) {
                                                                        element.parentElement.style = `margin-right: 5px;opacity: 0.6;filter: alpha(opacity = 60);flex: 1 0 25%;`
                                                                        return
                                                                    }
                                                                    const element2 = document.getElementById(name + "-2")
                                                                    element.value = current.name
                                                                    element2.value = current.url
                                                                    element.addEventListener("change", () => {
                                                                        current.name = element.value
                                                                        navigationSettings.custom[name] = current
                                                                    })
                                                                    element.addEventListener("blur", () => {
                                                                        setSetting("Navigation Buttons", navigationSettings)
                                                                    })
                                                                    element2.addEventListener("change", () => {
                                                                        let val = element2.value
                                                                        if (!val.includes("roblox.com") && val.length > 0) {
                                                                            val = "https://roblox.com/" + val
                                                                        }
                                                                        current.url = val
                                                                        navigationSettings.custom[name] = current
                                                                    })
                                                            ...
                                                            const boxShadow = "inset 0px 0px 11px 0px #1f87ff, 0px 0px 10px 2px #0b325e, 0px 0px 5px 7px rgb(0 0 0 / 0%)";
                                                            ...
                                                            element.addEventListener('click', async () => {
                                                                ...
                                                                if (preType) {
                                                                    ...
                                                                    if (document.getElementById(currentSelected[preType - 1])) document.getElementById(currentSelected[preType - 1])
                                                                        .style.boxShadow = "";
                                                                    ...
                                                                    if (preType == 2) {
                                                                        if (currentSetting[preType] == "none") {
                                                                            ...
                                                                            setImageBG(image, currentSelected)
                                                                        ...
                                                                        if (currentSetting[preType] == "none") {
                                                                            $("#rbx-body")
                                                                                .attr("rg-touched", false)
                                                                            $("#rbx-body")
                                                                                .removeClass(option.list[lastSetting].style)
                                                                        } else {
                                                                            ...
                                                                            $("#rbx-body")
                                                                                .addClass(option.list[name].style)
                                                                            $("#rbx-body")
                                                                                .attr("rg-touched", true)
                                                                        }
                                                                    }
                                                            ...
                                                            if (item.cf) {
                                                                ...
                                                                cPicker.onChange = async () => {
                                                                    ...
                                                                    if (element.getAttribute("property") == "color") {
                                                                        ...
                                                                        document.querySelector(":root")
                                                                            .style.setProperty("--tc-color", col)
                                                                    } else {
                                                                    ...
                                                                    $("#rbx-body")
                                                                        .attr("rg-touched", false)
                                                                    for (let child of document.getElementById("Gold")
                                                                ...
                                                                const fileModal = new Modal("FileModal", "Choose File", `Enter a Roblox image URL (upload under the create page) or upload your own file.`, "For higher quality, use a Roblox URL.", true)
                                                                ...
                                                                fileModal.addButton(`<input placeholder="URL" class="form-control input-field" value="${currentSetting[3][selected]}" style="cursor: pointer;">`, "FileURL", async (button) => {
                                                                    ...
                                                                    input.addEventListener("blur", async () => {
                                                                        ...
                                                                        if (url.includes("https://")) {
                                                                            if (url.includes("roblox.com/library/")) {
                                                                                const id = url.match(/\d+/)[0]
                                                                                await get(`https://thumbnails.roblox.com/v1/assets?assetIds=${id}&size=768x432&format=Png&isCircular=false`)
                                                                                await sleep(500)
                                                                                const resp = await get(`https://thumbnails.roblox.com/v1/assets?assetIds=${id}&size=768x432&format=Png&isCircular=false`)
                                                                                url = resp?.data?.[0]?.state == "Completed" ? resp.data[0].imageUrl : ""
                                                                            }
                                                                            console.log(url)
                                                                            ...
                                                                            image.onload = async () => {
                                                                                setImageBG(url, currentSelected)
                                                        ...
                                                        <span id="file-error" class="error"></span>`, "FileUpload", async (button) => {
                                                                            ...
                                                                            if (FileReader && files && files.length) {
                                                                                ...
                                                                                fr.onload = async () => {
                                                                                    ...
                                                                                    img.onload = async () => {
                                                                                        ...
                                                                                        for (let child of document.getElementById("Banana")
                                                                                                .parentElement.children) {
                                                                                            if (child.style.boxShadow) {
                                                                                                child.style.boxShadow = ""
                                                                                            }
                                                                                        ...
                                                                                        setImageBG(scaled, currentSelected)
                                                                                    }
                                                                                }
                                                                ...
                                                                for (const optionNew of item.options) {
                                                                    ...
                                                                    if (currentSelected[2][name].toLowerCase() == optionNew.toLowerCase()) {
                                                                        optionElement.setAttribute('selected', "selected")
                                                                    }
                                                                ...
                                                                document.getElementById(name)
                                                                    .addEventListener('change', async () => {
                                                                        let selectedInfo = document.getElementById(name)
                                                                            .getElementsByTagName("option")[document.getElementById(name)
                                                                                .selectedIndex].getAttribute('label')
                                                                        if (selectedInfo == "Normal") {
                                                                            document.querySelector(item.sel)
                                                                                .style[item.prop] = ""
                                                                        } else {
                                                                            document.querySelector(item.sel)
                                                                                .style[item.prop] = selectedInfo.toLowerCase()
                                                                        }
                                            ...
                                            if (key == "Navigation Buttons") {
                                                let isEnabled = navigationSettings?.enabled == null && setting.default || navigationSettings?.enabled
                                                toggleButton(key, isEnabled)
                                                addToggler(key, isEnabled, async () => {
                                                    toggleButton(key, !isEnabled)
                                                    isEnabled = !isEnabled
                                                    navigationSettings.enabled = isEnabled
                                                    await setSetting(key, navigationSettings)
                                                }, setting)
                                            } else {
            ...
            setTimeout(async () => {
                ...
                if (!hasExperiments) {
                    first('.rogold-Experimental .section-content', exp => {
                        getLabel("This will be free soon!", '', exp);
                ...
                if (!hasMoreNav) {
                    getLabel("Get 6 More Navigation Buttons!",
                        "https://www.roblox.com/game-pass/67271922/6-Navigation-Buttons-PRE-ORDER", await first('[id="Navigation Buttons-holder"] > div:nth-child(5) > a'))
                }
            }, 100)
        }
    ...
    jscolor.presets.default = {
        ...
        width: 136,
        height: 110,
        controlBorderColor: 'rgba(12,12,12,0.87)',
        sliderSize: 13,
        palette: [
        ...
        return new Promise(resolve => {
            ...
            while (jsonstr.length > 0) {
                ...
                for (let j = 0; j < chrome.storage.sync.QUOTA_BYTES_PER_ITEM; j++) {
                    const jsonLength = JSON.stringify(segment)
                        .length;
